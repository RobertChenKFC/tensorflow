...
from tensorflow.lite.python.convert import (
    convert_saved_model as _convert_saved_model,
)
from tensorflow.lite.python.convert import (
    ConverterError,
)  # pylint: disable=unused-import
...
from tensorflow.lite.python.util import (
    model_input_signature as _model_input_signature,
)
from tensorflow.lite.python.util import (
    modify_model_io_type as _modify_model_io_type,
)
from tensorflow.lite.python.util import (
...
class QuantizationMode(object):
    ...
    def __init__(
        self, optimizations, target_spec, representative_dataset, graph_def
    ):
    ...
    def post_training_fp16(self):
        return (
            self._any_optimization_enabled()
            and self._smallest_supported_type() == _dtypes.float16
        )
    ...
    def flags_modify_model_io_type(self, input_ty=None, output_ty=None):
        ...
        if self.is_integer_quantize():
            return {
                "inference_input_type": input_ty
                if input_ty
                else _dtypes.float32,
                "inference_output_type": output_ty
                if output_ty
                else _dtypes.float32,
            }
        else:
    ...
    def _validate_int8_required(self):
        ...
        if self._target_spec.supported_types and (
            ...
            raise ValueError(
                "TFLITE_BUILTINS_INT8 requires smallest supported "
                "type to be INT8."
            )
    ...
    def _is_allow_float(self):
        return (
            OpsSet.TFLITE_BUILTINS in set(self._target_spec.supported_ops)
        ) or (OpsSet.SELECT_TF_OPS in set(self._target_spec.supported_ops))
    ...
    def _parse_saved_model_args(self, always_enable_saved_model_import=False):
        ...
        if self.saved_model_dir:
            ...
            self._saved_model_version = (
                saved_model_proto.saved_model_schema_version
            )
...
class TFLiteConverterBaseV2(TFLiteConverterBase):
    ...
    def _save_conversion_params_metric(
        ...
        super(TFLiteConverterBaseV2, self)._save_conversion_params_metric(
            converter_kwargs, graph_def, inference_type, inference_input_type
        )
...
class TFLiteSavedModelConverterV2(TFLiteConverterBaseV2):
    ...
    def convert(self):
        ...
        """
        converter_kwargs = {
            "enable_tflite_resource_variables": self._enable_tflite_resource_variables
        }
        ...
        if self.saved_model_dir is None or not self.experimental_new_converter:
            ...
            self.saved_model_dir = None
            conversion_result = super(
                TFLiteSavedModelConverterV2, self
            ).convert(graph_def, input_tensors, output_tensors)
            self._increase_conversion_success_metric(conversion_result)
            return conversion_result

        if self._trackable_obj is None:
            self._debug_info = _get_debug_info(
                _build_debug_info_func(self._funcs[0].graph),
        ...
        self._save_conversion_params_metric(
            converter_kwargs, meta_graph.graph_def
        )
...
class TFLiteKerasModelConverterV2(TFLiteConverterBaseV2):
    ...
    def _convert_as_saved_model(self):
        ...
        try:
            ...
            if self.saved_model_dir:
                graph = _ops.Graph()
                saved_model = _loader_impl.SavedModelLoader(
                    self.saved_model_dir
                )
                saved_model.load_graph(graph, tags=self._saved_model_tags)
                meta_graph = saved_model.get_meta_graph_def_from_tags(
    ...
    def convert(self):
        ...
        result = super(TFLiteKerasModelConverterV2, self).convert(
...
class TFLiteFrozenGraphConverterV2(TFLiteConverterBaseV2):
    ...
    def convert(self):
        ...
        result = super(TFLiteFrozenGraphConverterV2, self).convert(
            graph_def, input_tensors, output_tensors
        )
        self._increase_conversion_success_metric(result)
        return result
...
class TFLiteConverterBaseV1(TFLiteConverterBase):
    ...
    def convert(self):
        ...
        if not self.saved_model_dir:
            ...
            if not quant_mode.contains_training_quant_op():
                try:
                    ...
                    graph_def = (
                        _convert_to_constants.disable_lower_using_switch_merge(
                            optimized_graph
                        )
                    )
        ...
        self._debug_info = _get_debug_info(
            self._debug_info_func, optimized_graph
        )
        ...
        self._validate_quantized_input_stats(
            converter_kwargs, calibrate_quantize
        )
        self._validate_experimental_new_quantizer_flag()
...
class TFLiteKerasModelConverter(TFLiteConverterBaseV1):
    ...
    def __init__(
        ...
        if context.executing_eagerly():
            ...
            frozen_func = (
                _convert_to_constants.convert_variables_to_constants_v2(
    ...
    def _convert_as_saved_model(self):
        ...
        try:
            ...
            result = _freeze_saved_model(
                temp_dir, None, None, None, tag_set, signature_key
            )
